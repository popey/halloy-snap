...
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/halloy-snap-check-packaging.XXXXXX")"
...
case "$success_output" in
  *"OK: halloy -> bin/launch is present and executable"*) : ;;
  *)
...
case "$absolute_output" in
  *"Skipping absolute command path for halloy: /snap/bin/halloy"*) : ;;
  *)
...
missing_output="$WORKDIR/missing.out"
if ROOT_DIR="$missing_fixture" bash "$SCRIPT" >"$missing_output" 2>&1; then
  echo "expected missing launcher fixture to fail"
  cat "$missing_output"
  exit 1
fi
if ! grep -q "ERROR: launcher not found for halloy: $missing_fixture/snap/local/bin/missing" "$missing_output"; then
...
if ! grep -q "ERROR: launcher is not executable for halloy: $nonexec_fixture/snap/local/bin/launch" "$nonexec_output"; then
